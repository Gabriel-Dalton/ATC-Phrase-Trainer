// ============================================================
// DATA — airlines, procedures, phonetics, ATC speech formatting
// ============================================================

const AIRLINES = [
  { icao: "ACA", radio: "Air Canada", types: ["A320", "A321", "B38M", "B77W", "B789"] },
  { icao: "WJA", radio: "WestJet", types: ["B737", "B38M", "B789"] },
  { icao: "JZA", radio: "Jazz", types: ["DH8D", "CRJ9"] },
  { icao: "ASA", radio: "Alaska", types: ["B739", "E175"] },
  { icao: "UAL", radio: "United", types: ["B739", "A320"] },
  { icao: "DAL", radio: "Delta", types: ["A220", "B739"] },
  { icao: "POE", radio: "Porter", types: ["E195"] },
  { icao: "SWG", radio: "Sunwing", types: ["B38M"] },
  { icao: "PCO", radio: "Pasco", types: ["SF34"] },
  { icao: "KLM", radio: "KLM", types: ["B789"] },
  { icao: "DLH", radio: "Lufthansa", types: ["A359"] },
  { icao: "CPA", radio: "Cathay", types: ["A359", "B77W"] },
  { icao: "ANA", radio: "All Nippon", types: ["B789"] },
  { icao: "EVA", radio: "Eva", types: ["B77W"] },
  { icao: "QFA", radio: "Qantas", types: ["B789"] },
];

const DESTINATIONS = [
  { name: "Toronto", fix: "GRIZZ" },
  { name: "Calgary", fix: "GRIZZ" },
  { name: "Edmonton", fix: "GRIZZ" },
  { name: "Winnipeg", fix: "GRIZZ" },
  { name: "Ottawa", fix: "GRIZZ" },
  { name: "Montreal", fix: "GRIZZ" },
  { name: "Seattle", fix: "SEA" },
  { name: "San Francisco", fix: "HUH" },
  { name: "Los Angeles", fix: "HUH" },
  { name: "Kelowna", fix: "GRIZZ" },
  { name: "Victoria", fix: "VARDI" },
  { name: "Prince George", fix: "CANUC" },
  { name: "Tokyo", fix: "VARDI" },
  { name: "Hong Kong", fix: "VARDI" },
  { name: "London", fix: "GRIZZ" },
];

const ORIGINS = [
  "Toronto", "Calgary", "Edmonton", "Seattle", "San Francisco",
  "Kelowna", "Victoria", "Winnipeg", "Los Angeles", "Montreal",
];

// Real-world flavoured CYVR SIDs (simplified)
const SIDS = [
  { name: "GRIZZ", number: 7, spoken: "GRIZZ seven" },
  { name: "RICHMOND", number: 6, spoken: "Richmond six" },
  { name: "VARDI", number: 4, spoken: "VARDI four" },
  { name: "CANUC", number: 5, spoken: "CANUC five" },
];

const FREQS = {
  atis: { name: "Vancouver ATIS", mhz: "124.60" },
  clearance: { name: "Vancouver Clearance", mhz: "121.40" },
  ground: { name: "Vancouver Ground", mhz: "121.70" },
  towerNorth: { name: "Vancouver Tower", mhz: "118.70" },
  towerSouth: { name: "Vancouver Tower", mhz: "119.55" },
  departure: { name: "Vancouver Departure", mhz: "126.12" },
  arrival: { name: "Vancouver Arrival", mhz: "133.10" },
  centre: { name: "Vancouver Centre", mhz: "133.05" },
};

// East flow (08s) or west flow (26s)
const FLOWS = {
  east: {
    label: "EAST FLOW",
    dep: "08L", arr: "08R",
    depHeading: 83, arrHeading: 83,
    windDir: [60, 110],
  },
  west: {
    label: "WEST FLOW",
    dep: "26R", arr: "26L",
    depHeading: 263, arrHeading: 263,
    windDir: [230, 290],
  },
};

const TAXI_ROUTES = {
  east: [
    ["Alpha", "Kilo"],
    ["Alpha", "Golf"],
    ["Juliett", "Alpha"],
  ],
  west: [
    ["Mike", "Lima"],
    ["Juliett", "Mike"],
    ["Alpha", "Mike"],
  ],
};

const PHONETIC = {
  A: "Alpha", B: "Bravo", C: "Charlie", D: "Delta", E: "Echo", F: "Foxtrot",
  G: "Golf", H: "Hotel", I: "India", J: "Juliett", K: "Kilo", L: "Lima",
  M: "Mike", N: "November", O: "Oscar", P: "Papa", Q: "Quebec", R: "Romeo",
  S: "Sierra", T: "Tango", U: "Uniform", V: "Victor", W: "Whiskey",
  X: "X-ray", Y: "Yankee", Z: "Zulu",
};

// ------------------------------------------------------------
// ATC speech formatting — numbers the way controllers say them
// ------------------------------------------------------------

const DIGIT_WORDS = ["zero", "one", "two", "three", "four", "five", "six", "seven", "eight", "niner"];

function sayDigits(str) {
  return String(str)
    .split("")
    .map((ch) => (ch === "." ? "decimal" : DIGIT_WORDS[Number(ch)] ?? ch))
    .join(" ");
}

function sayAltitude(ft) {
  if (ft >= 18000) {
    return "flight level " + sayDigits(String(Math.round(ft / 100)));
  }
  const thousands = Math.floor(ft / 1000);
  const hundreds = Math.floor((ft % 1000) / 100);
  let out = "";
  if (thousands) out += sayDigits(String(thousands)) + " thousand";
  if (hundreds) out += (out ? " " : "") + DIGIT_WORDS[hundreds] + " hundred";
  return out || "zero";
}

function sayHeading(hdg) {
  return sayDigits(String(hdg).padStart(3, "0"));
}

function sayFreq(mhz) {
  return sayDigits(mhz);
}

function sayRunway(rwy) {
  const num = rwy.replace(/[LRC]/g, "");
  const side = rwy.endsWith("L") ? " left" : rwy.endsWith("R") ? " right" : rwy.endsWith("C") ? " centre" : "";
  return sayDigits(num) + side;
}

function sayWind(dir, kt) {
  return `wind ${sayDigits(String(dir).padStart(3, "0"))} at ${kt < 10 ? DIGIT_WORDS[kt] : sayDigits(String(kt))}`;
}

function sayAltimeter(setting) {
  return "altimeter " + sayDigits(setting.replace(".", ""));
}

// ------------------------------------------------------------
// Readback element pattern helpers (normalized-transcript space)
// ------------------------------------------------------------

function runwayPatterns(rwy) {
  const num = rwy.replace(/[LRC]/g, "");
  const short = String(Number(num));
  const side = rwy.endsWith("L") ? "left" : rwy.endsWith("R") ? "right" : "centre";
  const letter = rwy.slice(-1).toLowerCase();
  return [
    `${num} ${side}`, `${short} ${side}`,
    `${num}${letter}`, `${short}${letter}`,
    `${num} ${letter}`, `${short} ${letter}`,
  ];
}

function altitudePatterns(ft) {
  if (ft >= 18000) {
    const fl = String(Math.round(ft / 100));
    return [`flight level ${fl}`, `fl ${fl}`, `fl${fl}`, `level ${fl}`];
  }
  return [String(ft)];
}

function freqPatterns(mhz) {
  const pats = [mhz, mhz.replace(".", " ")];
  if (mhz.endsWith("0")) pats.push(mhz.slice(0, -1)); // 121.70 -> 121.7
  const [int, dec] = mhz.split(".");
  if (dec) pats.push(`${int}.${Number(dec)}`);
  return pats;
}

const RANKS = [
  { xp: 0, title: "STUDENT PILOT" },
  { xp: 150, title: "PRIVATE PILOT" },
  { xp: 400, title: "COMMERCIAL PILOT" },
  { xp: 800, title: "FIRST OFFICER" },
  { xp: 1500, title: "SENIOR FIRST OFFICER" },
  { xp: 2500, title: "CAPTAIN" },
  { xp: 4000, title: "TRAINING CAPTAIN" },
  { xp: 6000, title: "CHECK AIRMAN" },
];

const rand = (arr) => arr[Math.floor(Math.random() * arr.length)];
const randInt = (min, max) => min + Math.floor(Math.random() * (max - min + 1));
